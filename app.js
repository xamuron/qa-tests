...
const EXPLANATIONS = [
  "HTTP 301 Moved Permanently сообщает, что ресурс окончательно перемещен на другой адрес.",
  "Тип int предназначен для целых чисел без дробной части. Пример: <code>CREATE TABLE users (age INT);</code>",
  "SELECT FOR UPDATE ставит блокировку на выбранные строки до завершения транзакции. Пример: <code>BEGIN; SELECT * FROM orders WHERE id = 42 FOR UPDATE;</code>",
  "API-тестирование проверяет корректность и безопасность обмена данными между системами.",
  "В ответе HTTP 201 Created заголовок Location указывает адрес созданного ресурса.",
  ...
  "Символ * выбирает все столбцы указанной таблицы. Пример: <code>SELECT * FROM orders;</code>",
  ...
  "JOIN связывает строки из нескольких таблиц по заданному условию. Пример: <code>SELECT * FROM users u JOIN orders o ON o.user_id = u.id;</code>",
  ...
  "Агрегатная функция avg вычисляет среднее значение выбранного столбца. Пример: <code>SELECT AVG(score) FROM users;</code>",
  ...
  "Якорь ^ означает начало строки, а диапазон [A-Z] — заглавную латинскую букву. Пример: <code>SELECT * FROM users WHERE REGEXP_LIKE(name, '^[A-Z]');</code>",
  ...
  "Подчеркивание заменяет ровно один символ, поэтому _o% означает букву o на втором месте. Пример: <code>SELECT * FROM users WHERE name LIKE '_o%';</code>",
  ...
  app.innerHTML = `
    <section class="quiz-layout" aria-labelledby="question-title">
      <div class="quiz-card">
        ...
        ${answered ? `<p class="feedback ${isCorrect ? "" : "wrong"}" role="status"><strong>${isCorrect ? "Верно." : "Неверно."}</strong> ${isCorrect ? explanation : `Правильный ответ: ${answers[correctIndex]}. ${explanation}`}</p>` : ""}
